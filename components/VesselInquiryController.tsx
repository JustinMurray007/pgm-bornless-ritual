...
import { VESSEL_SCRIPT } from '@/lib/vesselScript';
...
function buildWordEntries(): WordEntry[] {
  ...
  VESSEL_SCRIPT.forEach((section, sectionIdx) => {
  ...
  return (
    <div className="vessel-sticky-bar" role="toolbar" aria-label="Document playback controls">
      <button
        ...
        aria-label={playing ? 'Stop full document playback' : 'Play entire Vessel Inquiry'}
      >
        {playing ? (
          <>
...
export default function VesselInquiryController() {
  const isPlayingRef = useRef(false);
  const abortRef = useRef<AbortController | null>(null);
  const [playing, setPlaying] = useState(false);

  const sectionRefs = useRef<(PhoneticSectionHandle | null)[]>(
    Array(VESSEL_SCRIPT.length).fill(null)
  );
